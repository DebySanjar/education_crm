...
import { SkeletonStudents } from '../components/Skeleton'
...
export default function Students() {
  ...
  const filtered = students.filter(s => {
    ...
    const matchGroup = selectedGroup === 'Barchasi' || s.group === selectedGroup
    return matchSearch && matchGroup
  })

  // Count students per group
  const getGroupCount = (groupName) => {
    return students.filter(s => s.group === groupName).length
  }
  ...
  const openEdit = (s) => { 
    setForm({ 
      ...s, 
      contractSum: s.contractSum.toString(), 
  ...
  const handlePhoneChange = (value) => {
    ...
    if (cleaned.length === 13) {
      ...
      setErrors(e => ({ ...e, phone: 'Telefon raqami 9 ta raqamdan iborat bo\'lishi kerak' }))
    }
  }

  // Contract sum change handler
  const handleContractSumChange = (value) => {
    const formatted = formatNumberInput(value)
    setForm(f => ({ ...f, contractSum: formatted }))
    
    const num = parseFormattedNumber(formatted)
    if (num < 100000) {
      setErrors(e => ({ ...e, contractSum: 'Minimal summa: 100.000 so\'m' }))
    } else {
      setErrors(e => ({ ...e, contractSum: '' }))
    }
  }

  // Paid sum change handler
  const handlePaidChange = (value) => {
    const formatted = formatNumberInput(value)
    setForm(f => ({ ...f, paid: formatted }))
    
    const num = parseFormattedNumber(formatted)
    const contractNum = parseFormattedNumber(form.contractSum)
    
    if (num > contractNum) {
      setErrors(e => ({ ...e, paid: 'Shartnoma summasidan oshmasligi kerak' }))
    } else {
      setErrors(e => ({ ...e, paid: '' }))
    }
  }
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.fullName.trim()) {
      newErrors.fullName = 'Ism-sharifni kiriting'
    }
    
    if (form.phone.length !== 13) {
      newErrors.phone = 'Telefon raqami to\'liq emas'
    }
    
    const contractNum = parseFormattedNumber(form.contractSum)
    if (contractNum < 100000) {
      newErrors.contractSum = 'Minimal summa: 100.000 so\'m'
    }
    
    const paidNum = parseFormattedNumber(form.paid)
    if (paidNum > contractNum) {
      newErrors.paid = 'Shartnoma summasidan oshmasligi kerak'
    }
    ...
    const saved = { 
      ...form, 
      contractSum: contractNum, 
      paid: paidNum 
    }
    
    if (editId) {
      updateStudent(editId, saved)
      setShowModal(false)
    } else {
      addStudent(saved)
      setShowModal(false)
      setContractDialog(saved)
    }
  }
  ...
  const handleDownloadContract = async (student) => {
    try {
      ...
      toast.error('Shartnoma yaratishda xato yuz berdi: ' + err.message)
    }
  ...
  const exportExcel = async () => {
    ...
    sheet.columns = [
      { key: 'name',      width: 28 },
      { key: 'phone',     width: 16 },
      { key: 'passport',  width: 12 },
      { key: 'med',       width: 14 },
      { key: 'group',     width: 14 },
      { key: 'contract',  width: 18 },
      { key: 'paid',      width: 18 },
      { key: 'debt',      width: 18 },
    ...
    const headerRow = sheet.addRow([
      "Ism-Sharif", "Telefon", "Pasport", "Medspr.", "Guruh",
      "Shartnoma (so'm)", "To'langan (so'm)", "Qarz (so'm)", "Qo'shilgan sana", "Holat"
    ])
    headerRow.height = 22
    ...
    students.forEach((s, idx) => {
      ...
      const row = sheet.addRow([
        s.fullName, s.phone, s.passport, s.medStatus, s.group,
        s.contractSum, s.paid, s.contractSum - s.paid, s.joinDate, s.status
      ])
      row.height = 20
      row.eachCell((cell, col) => {
        ...
        if (col === 8 && s.contractSum - s.paid > 0) {
    ...
    saveAs(new Blob([buffer]), "oquvchilar.xlsx")
  }

  if (loading) return <Wrapper><SkeletonStudents /></Wrapper>
        ...
        <Table>
          <thead>
            <tr>
              ...
              <Th>Medspr.</Th><Th>Guruh</Th><Th>Shartnoma</Th><Th>To'langan</Th><Th>Qarz</Th><Th>Amallar</Th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={9}><EmptyRow>O'quvchi topilmadi</EmptyRow></td></tr>
            )}
            {filtered.map((s, i) => {
              ...
              return (
                <tr key={s.id}>
                  ...
                  <Td>
                    <ActionBtns>
                      {isSuperAdmin() && (
                        <IconBtn $color="#00e0ff" onClick={() => openEdit(s)} title="Tahrirlash"><MdEdit /></IconBtn>
                      )}
                      <IconBtn $color="#a78bfa" onClick={() => setContractDialog(s)} title="Shartnoma"><MdDescription /></IconBtn>
                      {isSuperAdmin() && (
                        <IconBtn $color="#ff6b6b" onClick={() => setDeleteConfirm(s.id)} title="O'chirish"><MdDelete /></IconBtn>
                      )}
                    </ActionBtns>
                  </Td>
                </tr>
              ...
              <FormGrid>
                <FormGroup>
                  ...
                  <input 
                    ...
                    onChange={e => {
                      ...
                      if (e.target.value.trim()) setErrors(e => ({ ...e, fullName: '' }))
                    }} 
                ...
                <FormGroup>
                  ...
                  <select 
                    ...
                    onChange={e => setForm(f => ({ ...f, medStatus: e.target.value }))}
                  >
                ...
                <FormGroup>
                  ...
                  {groups.length === 0 && <HintText>Avval guruh qo'shing</HintText>}
                </FormGroup>
      ...
      {contractDialog && (
        <ModalOverlay onClick={() => setContractDialog(null)}>
          <ContractDialogBox onClick={e => e.stopPropagation()}>
            ...
            <CloseContractBtn onClick={() => setContractDialog(null)}>
              <MdClose /> Yopish
            </CloseContractBtn>
          </ContractDialogBox>
        </ModalOverlay>
...
`
const FiltersRow = styled.div`display: flex; align-items: center; gap: 16px; flex-wrap: wrap;`

const SearchBox = styled.div`
  display: flex; align-items: center; gap: 8px;
  background: #13161f; border: 1px solid #2d3748; border-radius: 8px;
  padding: 10px 16px; flex: 1; max-width: 500px;
...
const GroupChip = styled.div`
  ...
  .count {
    ...
    color: ${({ $active }) => $active ? '#000' : '#e2e8f0'};
    padding: 2px 8px;
    border-radius: 10px;
    font-size: 0.8rem;
    font-weight: 700;
...
const ConfirmModal = styled.div`
  ...
  h3 { font-size: 1.1rem; font-weight: 600; color: #e2e8f0; margin-bottom: 10px; }
...
/* ── Contract Dialog ── */
const ContractDialogBox = styled.div`
...
const CardSvg1 = styled.svg`
  position: absolute; bottom: -2px; right: -80px;
  width: 192px; height: 192px; z-index: 10; margin: -8px;
  fill: #f9fafb; stroke: #1e3a5f;
...
const CardSvg2 = styled.svg`
  position: absolute; bottom: -2px; right: -80px;
  width: 192px; height: 192px; z-index: 10; margin: -8px;
  fill: #f9fafb; stroke: #1e6090;
  transition: transform 0.2s;
  ${ContractCard}:hover & { transform: scale(1.25); }
`

const CloseContractBtn = styled.button`
  display: flex; align-items: center; gap: 6px;
  background: #1a1d2e; border: 1px solid #2d3748; color: #8892b0;
  padding: 9px 20px; border-radius: 8px; cursor: pointer; font-size: 0.9rem;
  &:hover { color: #e2e8f0; }
